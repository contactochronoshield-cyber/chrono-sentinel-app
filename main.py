AMBER = (0.95, 0.7, 0.1, 1) GREEN = (0.2, 0.9, 0.4, 
1) RED = (0.95, 0.25, 0.25, 1) FG = (0.85, 0.9, 
0.92, 1)

IS_PRO = False REFRESH_SECONDS = 4
...
class SentinelRoot(BoxLayout): def __init__(self, 
    **kwargs):
        super().__init__(orientation="vertical", 
        padding=16, spacing=10, **kwargs)

        with self.canvas.before: Color(*BG) 
            self.bg_rect = 
            Rectangle(size=Window.size, 
            pos=self.pos)
        Window.bind(size=self._update_bg)

        self.add_widget(Label( text="CHRONO 
            SENTINEL", color=CYAN, bold=True, 
            font_size=24, size_hint_y=None, 
            height=40
        )) self.status_lbl = 
        Label(text="Desconectado", color=AMBER, 
        size_hint_y=None, height=24) 
        self.add_widget(self.status_lbl)

        self.server_input = TextInput( 
            hint_text="http://IP:5000/api/status", 
            multiline=False, size_hint_y=None, 
            height=44, background_color=(0.1, 0.12, 
            0.14, 1), foreground_color=FG
        ) self.add_widget(self.server_input)

        connect_btn = Button(text="Conectar", 
                              size_hint_y=None, 
                              height=44, 
        ...
        self.rows = { "hostname": StatRow("Nodo"), 
            "cpu": StatRow("CPU"), "ram": 
            StatRow("RAM"), "disk": 
            StatRow("Disco"), "net": 
            StatRow("Red"),
